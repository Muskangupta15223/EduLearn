...
public class CourseController {
  ...
  public ResponseEntity<CourseDto> create(
    ...
    return ResponseEntity.ok(
      CourseMapper.toDto(
        courseService.createCourse(course, userId, userName, role)
      )
    );
  }
  ...
  public ResponseEntity<List<CourseDto>> getAll(
    ...
    return ResponseEntity.ok(
      CourseMapper.toDtoList(courseService.getAllCourses(userId, role))
    );
  }
  ...
  public ResponseEntity<List<CourseDto>> getFeatured() {
    return ResponseEntity.ok(
      CourseMapper.toDtoList(courseService.getFeaturedCourses())
    );
  }

  @GetMapping("/category/{cat}")
  public ResponseEntity<List<CourseDto>> getByCategory(
    @PathVariable String cat
  ) {
    return ResponseEntity.ok(
      CourseMapper.toDtoList(courseService.getCoursesByCategory(cat))
    );
  }
  ...
  public ResponseEntity<List<CourseDto>> search(
    ...
    return ResponseEntity.ok(
      CourseMapper.toDtoList(courseService.searchCourses(query))
    );
  }
  ...
  public ResponseEntity<List<CourseDto>> getMyCreated(
    ...
    return ResponseEntity.ok(
      CourseMapper.toDtoList(courseService.getMyCreatedCourses(userId, role))
    );
  }
  ...
  public ResponseEntity<List<CourseDto>> getPending(
    ...
    return ResponseEntity.ok(
      CourseMapper.toDtoList(courseService.getPendingCourses(role))
    );
  }
  ...
  public ResponseEntity<CourseDto> update(
    ...
    return courseService
      .updateCourse(id, courseDetails, userId, role)
      .map(CourseMapper::toDto)
      .map(ResponseEntity::ok)
      .orElse(ResponseEntity.notFound().build());
  }
  ...
  public ResponseEntity<CourseDto> reject(
    ...
    return courseService
      .rejectCourse(id, body.get("reason"), userId, role)
      .map(CourseMapper::toDto)
      .map(ResponseEntity::ok)
      .orElse(ResponseEntity.notFound().build());
  }
  ...
  public ResponseEntity<CourseDto> unpublish(
    ...
    return courseService
      .unpublishCourse(id, userId, role)
      .map(CourseMapper::toDto)
      .map(ResponseEntity::ok)
      .orElse(ResponseEntity.notFound().build());
  }
  ...
  public ResponseEntity<CourseDto> uploadThumbnail(
    ...
    return courseService
      ...
      .map(course -> {
        ...
        return ResponseEntity.ok(
          CourseMapper.toDto(
            courseService.updateCourse(id, course, userId, role).get()
          )
        );
      })
      .orElse(ResponseEntity.notFound().build());
  ...
  public ResponseEntity<List<java.util.Map<String, Object>>> getInstructorAnalytics(
    ...
    List<Course> allCourses = courseService.getAllCoursesForAdminAnalytics(
      role
    );
    ...
    for (Course c : allCourses) {
      ...
      java.util.Map<String, Object> stats = analyticsMap.getOrDefault(
        instrId,
      ...
      stats.put(
        "totalCourses",
        (int) stats.getOrDefault("totalCourses", 0) + 1
      );
      stats.put(
        "totalStudents",
        (int) stats.getOrDefault("totalStudents", 0) +
        (c.getStudentsCount() != null ? c.getStudentsCount() : 0)
      );
      stats.put(
        "publishedCourses",
        (int) stats.getOrDefault("publishedCourses", 0) +
        (CourseConstants.STATUS_PUBLISHED.equals(c.getStatus()) ? 1 : 0)
      );
