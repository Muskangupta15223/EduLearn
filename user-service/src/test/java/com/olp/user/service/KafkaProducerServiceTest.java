...
class KafkaProducerServiceTest {
  ...
  void setUp() {
    kafkaProducerService =
      new KafkaProducerService(kafkaTemplate, new ObjectMapper());
  }

  @Test
  void sendUserSignupEvent_sendsToKafka() {
    kafkaProducerService.sendUserSignupEvent(
      1L,
      "user@test.com",
      "Test User",
      "http://avatar.jpg"
    ...
    ArgumentCaptor<String> messageCaptor = ArgumentCaptor.forClass(
      String.class
    );
  ...
  void sendUserRoleUpdatedEvent_sendsToKafka() {
    kafkaProducerService.sendUserRoleUpdatedEvent(
      2L,
      "prof@test.com",
      "Professor",
      "INSTRUCTOR"
  ...
  void sendUserProfileUpdatedEvent_sendsToKafka() {
    ...
    verify(kafkaTemplate)
      .send(eq("user-events"), eq("3"), messageCaptor.capture());
