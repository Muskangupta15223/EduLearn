...
import com.olp.auth.dto.AuthDtos.AuthResponse;
import com.olp.auth.dto.AuthDtos.OAuthLoginSuccessResponse;
...
class AuthControllerTest {
  ...
  void register_success() {
    when(authUserRepository.findByEmail("user@test.com"))
    ...
    when(
      jwtUtil.generateToken(anyString(), anyString(), anyLong(), anyString())
    )
      .thenReturn("jwt-token");

    var request = new com.olp.auth.dto.AuthDtos.RegisterRequest(
      "user@test.com",
      "Test User",
      "password123",
      null
    ...
    assertEquals("jwt-token", ((AuthResponse) response.getBody()).token());
  }
  ...
  void register_duplicateEmail_returnsBadRequest() {
    when(authUserRepository.findByEmail("existing@test.com"))
  ...
  void register_withInstructorRole_setsInstructorRole() {
    when(authUserRepository.findByEmail("instructor@test.com"))
      .thenReturn(Optional.empty());
  ...
  void login_success() {
    ...
    when(passwordEncoder.matches("password", "$2a$hashedPass"))
      .thenReturn(true);
    when(
      jwtUtil.generateToken(anyString(), anyString(), anyLong(), anyString())
    )
  ...
  void login_userNotFound_returnsUnauthorized() {
    when(authUserRepository.findByEmail("nobody@test.com"))
      .thenReturn(Optional.empty());
  ...
  void changePassword_success() {
    ...
    var request = new com.olp.auth.dto.AuthDtos.ChangePasswordRequest(
      "oldPass",
      "newPass"
    );
  ...
  void updateProfile_success() {
    ...
    var request = new com.olp.auth.dto.AuthDtos.ProfileUpdateRequest(
      "New Name",
      null,
      "12345",
      "bio"
  ...
  void forgotPassword_unknownEmail_returnsOk() {
    when(authUserRepository.findByEmail("unknown@test.com"))
      .thenReturn(Optional.empty());

    var request = new com.olp.auth.dto.AuthDtos.ForgotPasswordRequest(
      "unknown@test.com"
    );
  ...
  @Test
  void login_plaintextPasswordMigratesToEncodedPassword() {
    AuthUser user = createUser(1L, "user@test.com", "Test", "STUDENT");
    user.setPassword("plain-pass");
    when(authUserRepository.findByEmail("user@test.com")).thenReturn(Optional.of(user));
    when(passwordEncoder.encode("plain-pass")).thenReturn("$2a$migrated");
    ...
    ResponseEntity<?> response = authController.login(
      new com.olp.auth.dto.AuthDtos.LoginRequest("user@test.com", "plain-pass")
    );

    assertEquals(HttpStatus.OK, response.getStatusCode());
    verify(authUserRepository, atLeastOnce()).save(user);
  }

  @Test
  void loginSuccess_existingUserWithNewPictureUpdatesAvatar() {
    ...
    when(oAuth2User.getAttribute("picture")).thenReturn("https://img/new.png");
    AuthUser user = createUser(3L, "user@test.com", "Old", "STUDENT");
    user.setAvatarUrl("https://img/old.png");
    when(authUserRepository.findByEmail("user@test.com")).thenReturn(Optional.of(user));
    when(authUserRepository.save(any(AuthUser.class))).thenAnswer(i -> i.getArgument(0));
    when(jwtUtil.generateToken(anyString(), anyString(), anyLong(), anyString())).thenReturn("oauth-jwt");

    ResponseEntity<?> response = authController.loginSuccess(oAuth2User);

    assertEquals(HttpStatus.OK, response.getStatusCode());
    assertEquals("oauth-jwt", ((OAuthLoginSuccessResponse) response.getBody()).token());
  }

  @Test
  void refresh_deactivatedUser_returnsForbidden() {
    AuthUser user = createUser(1L, "user@test.com", "Test", "DEACTIVATED");
    when(authUserRepository.findByEmail("user@test.com")).thenReturn(Optional.of(user));
  ...
  void resetPassword_nullFields_returnsBadRequest() {
    var request = new com.olp.auth.dto.AuthDtos.ResetPasswordRequest(
      null,
      null
    );
